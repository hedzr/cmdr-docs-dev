...
在 tiny0 示例中已经展示了极简的 cmdr-based app 的编写。但那只是玩具，接下来的几个小节我们依次介绍较为真实的用法。
...
为你的应用程序创建一条子命令使用的是两种序列：`app/b.Cmd("long","short",...)...Build()` 和 `app/b.Cmd("long","short",...)...With(cb)`。

前者使用 `Build()` 结束创建模式。

后者带有一个回调函数闭包。这样你就能在闭包中继续创建下一级子命令了。

创建一个选项参数的方式也是类似的，`app/b.Flag("long","short",...)...Build()` 和 `app/b.Flag("long","short",...)...With(cb)`。
其中，后者可以用于在闭包中设置 flag 的其他属性。但你依然可以使用流式调用风格，然后以 `Build()` 作为终结符完成构建模式。
