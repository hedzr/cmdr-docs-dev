...
date: 2024-02-03T05:00:00+08:00
last_modified_at: 2024-02-03T13:55:00+08:00
author: hedzr
tags: [golang, arch]
...
toc: true
header:
  teaser: /images/blog/image-20220202111728058.png
...
新年后补充一下，`go.work` 具有截断向上回溯的能力。

具体解说其含义即，如果在子目录中存在 go.work + go.mod 组合，那么其任何父目录中的 `go.work` 就不再被其纳入考虑范围。
如此，产生的效果是，父目录中的代码依据其对应的 `go.work` 去搜索相关联的本地文件夹中的代码库，而子目录中的代码则根据其私有的 `go.work` 的定义去进行搜索，两者所使用的依赖关系各自独立、互不相干。

这种独立性还可以被利用于构建复杂的依赖关系，在解决极端复杂的多线代码库管理时是非常有用的。

当然也要强调的是，这类奇技淫巧不适合于工程代码开发。所谓的库代码开发者和工程项目开发者所面对的场景是非常有区别的。工程上来说，没有复杂的依赖关系才是正确的追求，尤其是要避免依赖关系重叠和不一致的情况。
