...
- concise style by `[Create]` and cmd/xxcmd.go
- passing config arg style by `[cmdr.NewAppWithConfig()]`
- struct tag style: using `[cmdr.Create().BuildFrom()](https://pkg.go.dev/github.com/hedzr/cmdr/v2#Creator.BuildFrom)` to build command system from a struct value, see example [#example_Create_buildFromStructValue](https://pkg.go.dev/github.com/hedzr/cmdr/v2#example-Create-BuildFromAStructValue)
