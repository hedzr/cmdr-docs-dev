...
date: 2021-09-05T05:00:00+08:00
last_modified_at: 2021-09-05T07:13:00+08:00
author: hedzr
tags:
  [c++, c++11, c++17, builder pattern, design patterns, 构建者模式, 设计模式]
categories: c++ algorithm
comments: true
toc: true
header:
  teaser: /images/blog/2021-09-05-cxx17-builder-pattern/image-20210905153441992.png
...
> 已经写了一篇 [谈 C++17 里的 Factory 模式](./cxx17-factory-pattern/) ，后来又顺便肝了一篇 [谈 C++17 里的 Singleton 模式](./cxx17-singleton-pattern/) 。看来是得要整一大堆了，对于懒散的人来说这很麻烦。我不知道是不是要打算会写完整个 GoF 的个人理解以及新的的实现，慢慢看吧，做了就做了。
...
![生成器设计模式](/images/blog/2021-09-05-cxx17-builder-pattern/builder-zh.png)
...
Builder 模式是 [Creational Patterns](https://en.wikipedia.org/wiki/Creational_pattern) 中的一种。在 [谈 C++17 里的 Factory 模式](./cxx17-factory-pattern/) 中，我们已经介绍过创建型模式了，所以本文不再赘述了。
...
示例代码提供了一种编码结构上的刻板手段，即通过 `_model class_::builder()` 获得构建者，在最后一步时以 `builder.build()` 来获得最终的 _model class_ 实例对象。有时候刻板手段是最佳的选择。的确，稍后我们会看到一个 design pattern 其实现方法是可以多种多样的。然而保持编码结构的相似性，将会有利于使用者在探视接口 API 时，尤其是通过 namespace 层级探视可用的接口时，无需额外文档地获得接口使用方法。
