...
它是一个基于 C++17/C++20 的 header-only 软件包。
...
**NEWS**:  
We might rewrite `cmdr-cxx` with cxx-modules and
C++23 (with `import std` enabled).

**注意**：  
我们有可能准备启动工作重写 `cmdr-cxx`，迁移该工作到 C++23
上，使用新的 `import std` 特性，同时也产出 cxx-modules
库。

</Callout>

<Cards>
  <Card title="Cmdr(-go) v2 Documentation" href="../../docs/cmdr/v2/" />
  <Card title="Cmdr(-go) v1 Documentation" href="../../docs/cmdr/v1/" />
</Cards>
...
<Cards>
  {/* <Card title="`hedzr/cmdr`: cmdr in golang" href="../../docs/cmdr/v2" /> */}
  <Card
    title="`hedzr/Cmdr.Core`: cmdr in c# (old)"
    href="../../docs/Cmdr.Core/"
  />
</Cards>
