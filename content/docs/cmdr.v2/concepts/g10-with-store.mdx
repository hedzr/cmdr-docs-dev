...
[`hedzr/cmdr-loaders`](https://github.com/hedzr/cmdr-loaders) 提供了一个对 `cmdr.Create()` 更进一步的包装接口 `loaders.Create()`，它将会自动加载恰当位置的外部配置文件或者配置源。
...
总的来说，对于常规开发，我们推荐你使用 `cmdr.Create()` 和 `loaders.Create()` 这两个接口。
...
在 `OnAction` 回调函数中，`cmd.Store()` 返回一个完整 Store 的相对于当前命令的子集。对于 `jump to` 子命令，键值 `app.cmd.jump.to` 所下辖的子树将被返回。所以 `cmd.Store().MustBool("full")` 就可以直接取得 `--full` 的状态。
