...
import { gitConfig } from './repo';

// export const repo = 'hedzr';
// export const owner = 'cmdr-docs-dev';
...
async function getOctokit(): Promise<Octokit> {
  ...
  // console.log(`gitConfig is: ${gitConfig}, user: ${gitConfig.user}, repo: ${gitConfig.repo}`);
...
async function createDiscussionThread(pageId: string, body: string) {
  ...
  const category = destination.discussionCategories.nodes.find(
    (category) => category.name === gitConfig.feedbackCategory,
  );

  if (!category) throw new Error(`Please create a "${gitConfig.feedbackCategory}" category in GitHub Discussion`);
  ...
  if (discussion) {
    ...
    return {
      githubUrl: result.discussion.url||'#404',
    };
  }
}
