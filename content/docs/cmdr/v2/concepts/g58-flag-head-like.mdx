...
自 cmdr.v2 以来，我们优化了解析算法，因此 `head -1` 这个风格的标志不再受到限制。
这意味着你可以为每个命令关联一个此风格的标志选项，它被作为该命令的某个数值选项的快捷词，即 `app cmd1 cmd2 --count 5` 被缩写为 `app cmd1 cmd2 -5`。
按照逻辑，用户输入的 `-5` 首先与 `cmd2` 进行匹配，如果未能匹配则继续尝试 `cmd1` 以及 `app` Root 命令，直至匹配成功。
