...
End-user can pass command-line arguments with `-Wunused-variable` to enable `warnings.unused-variable` option (_above_), or disable it with `-Wno-unused-variable`. Another way is using short flag `-Wunused-variable` and `-Wno-unused-variable`.
