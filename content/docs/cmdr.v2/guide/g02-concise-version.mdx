...
在上一个示例中展示了从头开始创建 cmdr App。

为了向你阐释 cmdr 提供
的编程接口的用法，所以这个例子中遵循逐步推进的原则依次列举了每一个步
骤，包括建立 cmdr 的 `cli.App` 对象，向其添加子命令以及选项，最后
运行这个 `cli.App` 对象。

但实际上还可以使用 `Create()` 将其进一步简化和正规化。

不仅如此，`Create()` 返回的 Buidler 支持这样的方式：你可以提供一
组分立的子命令，然后通过 Builder 的 `WithAdders(...)` 接口将它们添加
到 `app.RootCommand` 中。
